...
class tasksCog(commands.Cog):
    ...
    async def tick(self):
        ...
        tick = str(self.index)
        print("\n[Reko] {"+str(datetime.datetime.now())+"} Tick: "+tick+f" {round(self.bot.latency * 1000)}ms")
        self.index += 1
    ...
    async def before_track(self):
        ...
        await self.bot.wait_until_ready()
    @track.error
    async def trackerror(self):
        print("[track]: Error in track task. Most probably a awaiting error.")
    @status.before_loop
